...
Headers / CDN (Vercel): estáticos `/_next/static` con cache largo **solo en producción** (`immutable`; filenames hasheados). En `next dev` Next.js ya envía `no-store` en estáticos — **no** hay header custom sobre `/_next/static` (rompe HMR). En HTML, `Clear-Site-Data: "cache"` evicta entradas `immutable` viejas (Turbopack reusa URLs de chunk). HTML de `(app)` dinámico (`private, no-store`) — no “arreglar” TTFB cacheando paneles.
