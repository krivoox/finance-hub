...
- **Authorized JavaScript origins** (mismo OAuth client) — necesarios para el flujo GIS `id_token` en la **PWA instalada** (`display: standalone`), que evita el redirect a Google (en iOS el redirect pierde cookies/sesión de la app). Incluir `http://localhost:3000`, `https://finance.krivoox.com`, y Previews usados.
