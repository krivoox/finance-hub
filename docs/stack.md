...
- **Authorized JavaScript origins** (mismo OAuth client) — necesarios para el flujo GIS `id_token` en la **PWA instalada** (`display: standalone`). En standalone **no** se usa redirect OAuth: en iOS la sesión quedaría en Safari y la app volvería a `/login`. Incluir `http://localhost:3000`, `https://finance.krivoox.com`, y Previews usados.
...
- `src/proxy.ts` (cookie `better-auth*`; convención Next 16, reemplaza `middleware.ts`)
